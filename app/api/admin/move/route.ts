...
export async function POST(req: NextRequest) {
  ...
  const { public_id, to_gallery } = await req.json()

  if (!public_id || !to_gallery) {
  ...
  const to_public_id = `${GALLERY_BASE}/${to_gallery}/${filename}`
  ...
  if (!res.ok) {
    ...
    console.error('[Move] Erro Cloudinary:', res.status, errText)
    return NextResponse.json({ error: `Cloudinary: ${res.status} — ${errText}` }, { status: 500 })
  }

  const renamed = await res.json()
  return NextResponse.json({ ok: true, to_public_id: renamed.public_id })
}
