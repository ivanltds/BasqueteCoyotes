...
// ─── TabMidiaCompleta (Mídia + Áudio em uma tela) ────────────────────────────

function TabMidiaCompleta() {
  return (
    <div className="space-y-16">
      <TabMidia />
      <div className="border-t border-b-stone/30 pt-12">
        <div className="mb-8">
          <span className="font-mono text-b-orange text-xs uppercase tracking-[0.3em] block mb-2">// playlists</span>
          <h2 className="font-display text-3xl uppercase text-white">Áudio</h2>
        </div>
        <TabAudio />
      </div>
    </div>
...
// ─── TabGaleriaFotos (Aprovações + Galerias + Fotos em uma tela) ──────────────

function TabGaleriaFotos() {
  const router = useRouter()
  ...
  // ── view ──
  const [view, setView]             = useState<'aprovacoes' | string>('aprovacoes')
  ...
  useEffect(() => { if (view !== 'aprovacoes') loadPhotos(view) }, [view, loadPhotos])

  // ── approve / reject ──
  function approveTarget(photo: PendingPhoto) {
  ...
  async function approve(photo: PendingPhoto) {
    const target = approveTarget(photo)
    ...
    if (res.ok) setPending(prev => prev.filter(p => p.public_id !== photo.public_id))
    else { const d = await res.json(); alert(d.error ?? 'Erro ao aprovar.') }
    setBusy(null)
  }
  ...
  // ── move / remove photo ──
  function moveTargetFor(photo: GalleryPhoto) {
    return moveTarget[photo.public_id] ?? galleries.find(g => g.folder_slug !== view)?.folder_slug ?? ''
  }

  async function movePhoto(photo: GalleryPhoto) {
    const to = moveTargetFor(photo)
    if (!to) return
    setBusy(photo.public_id)
    ...
    if (res.ok) { setPhotos(prev => prev.filter(p => p.public_id !== photo.public_id)); setMovingPhoto(null) }
    else { const d = await res.json(); alert(d.error ?? 'Erro ao mover.') }
    setBusy(null)
  }
  ...
  async function removePhoto(photo: GalleryPhoto) {
    ...
    if (res.ok) setPhotos(prev => prev.filter(p => p.public_id !== photo.public_id))
    setBusy(null)
  }

  // ── gallery CRUD ──
  function openCreate() { setInputName(''); setModalError(''); setModal('create') }
  function openEdit(g: Gallery) { setInputName(g.display_name); setModalError(''); setModal({ type: 'edit', gallery: g }) }
  function closeModal() { setModal(null) }

  async function handleCreate() {
    if (!inputName.trim()) { setModalError('Nome obrigatório.'); return }
    setSavingGallery(true); setModalError('')
    ...
    const data = await res.json()
    if (!res.ok) { setModalError(data.error ?? 'Erro.'); setSavingGallery(false); return }
    setSavingGallery(false); closeModal(); loadAll()
  }

  async function handleEdit() {
    if (!modal || modal === 'create') return
    const g = (modal as { gallery: Gallery }).gallery
    if (!inputName.trim()) { setModalError('Nome obrigatório.'); return }
    setSavingGallery(true); setModalError('')
    const res = await fetch(`/api/admin/galleries/${g.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ display_name: inputName }),
    })
  ...
  async function deleteGallery(g: Gallery) {
    ...
    if (view === g.folder_slug) setView('aprovacoes')
    loadAll()
  }
  ...
  const activeGallery  = galleries.find(g => g.folder_slug === view)
  const otherGalleries = galleries.filter(g => g.folder_slug !== view)
  ...
  return (
    <>
      ...
      <div className="flex gap-0 min-h-[60vh]">
        ...
        {/* ── Sidebar ── */}
        <nav className="w-44 shrink-0 border-r border-b-stone/20 pr-3 space-y-0.5 self-start sticky top-6">

          {/* Aprovações */}
          <button
            onClick={() => { setView('aprovacoes'); setMovingPhoto(null) }}
            className={`w-full text-left flex items-center justify-between px-3 py-2.5 transition-all ${
              view === 'aprovacoes'
                ? 'bg-b-orange/10 text-b-orange border-l-2 border-b-orange pl-[10px]'
                : 'text-gray-500 hover:text-white hover:bg-white/5'
            }`}
          >
            <span className="font-mono text-[11px] uppercase tracking-widest">Aprovações</span>
            {pending.length > 0 && (
              <span className="font-mono text-[10px] font-bold px-1.5 py-0.5 rounded bg-b-orange text-b-dark leading-none">
                {pending.length}
              </span>
            )}
          </button>

          <div className="border-t border-b-stone/20 my-2 mr-0" />

          {/* Galerias */}
          {galleries.map((g, idx) => (
            <div key={g.id} className="group relative">
              <button
                onClick={() => { setView(g.folder_slug); setMovingPhoto(null) }}
                className={`w-full text-left px-3 py-2 pr-8 transition-all ${
                  view === g.folder_slug
                    ? 'bg-b-neon/10 text-b-neon border-l-2 border-b-neon pl-[10px]'
                    : 'text-gray-500 hover:text-white hover:bg-white/5'
                }`}
              >
                <span className="font-mono text-[11px] uppercase tracking-widest truncate block">{g.display_name}</span>
                <span className="font-mono text-[9px] text-gray-700 normal-case tracking-normal">
              ...
              <div className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:flex flex-col">
                <button onClick={() => moveOrder(g, 'up')} disabled={idx === 0}
                  className="text-gray-600 hover:text-white disabled:opacity-20 leading-none text-[9px] px-1 py-0.5">▲</button>
          ...
          <div className="border-t border-b-stone/20 my-2" />

          {/* Nova galeria */}
          <button onClick={openCreate}
            className="w-full text-left px-3 py-2 font-mono text-[11px] uppercase tracking-widest text-gray-600 hover:text-b-neon transition-colors">
            + Nova galeria
          </button>
        </nav>
        ...
        <div className="flex-1 pl-6 min-w-0">
          ...
          {view === 'aprovacoes' && (
            <div>
              <div className="flex items-center gap-4 mb-6">
                <h3 className="font-display text-2xl uppercase text-white">Aprovações</h3>
              ...
              {pending.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {pending.map(photo => {
                    ...
                    const current = approveTarget(photo)
                    return (
                        ...
                        <div className="p-4 flex flex-col gap-3 flex-1">
                          <div>
                            ...
                            <select value={current}
                              ...
                              {galleries.map(g => <option key={g.folder_slug} value={g.folder_slug}>{g.display_name}</option>)}
                            </select>
                          </div>
          ...
          {view !== 'aprovacoes' && (
            <div>
              {/* Cabeçalho da galeria com ações */}
              <div className="flex items-center gap-3 mb-6 flex-wrap">
                <h3 className="font-display text-2xl uppercase text-white">{activeGallery?.display_name}</h3>
                {!loadingPhotos && (
                  <span className="font-mono text-xs text-gray-500">
                ...
                <div className="flex gap-2 ml-auto">
                  {activeGallery && (
                    <button onClick={() => openEdit(activeGallery)}
                      className="font-mono text-[11px] uppercase text-b-orange border border-b-orange/30 px-3 py-1 hover:border-b-orange transition-colors">
                      ✎ Renomear
                    </button>
                  )}
                  {activeGallery && (
                    <button onClick={() => deleteGallery(activeGallery)}
                      className="font-mono text-[11px] uppercase text-red-500 border border-red-900/30 px-3 py-1 hover:border-red-500 transition-colors">
                      ✕ Deletar
                    </button>
                  )}
                </div>
                          ...
                          {isMoving && otherGalleries.length > 0 && (
                            <div className="flex flex-col gap-2">
                              <select value={to}
                                ...
                                {otherGalleries.map(g => <option key={g.folder_slug} value={g.folder_slug}>{g.display_name}</option>)}
                              </select>
                              <div className="flex gap-2">
                                <button onClick={() => movePhoto(photo)} disabled={isBusy || !to}
                                ...
                                <button onClick={() => setMovingPhoto(null)} disabled={isBusy}
                                  className="px-3 border border-b-stone text-gray-500 font-mono text-xs hover:text-white disabled:opacity-40">✕</button>
                              </div>
                            </div>
                          )}
                          <div className="flex gap-2">
                            {otherGalleries.length > 0 && !isMoving && (
                              <button onClick={() => setMovingPhoto(photo.public_id)} disabled={isBusy}
                            ...
                            <button onClick={() => removePhoto(photo)} disabled={isBusy}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4" onClick={closeModal}>
          <div className="bg-b-gray border-2 border-b-stone w-full max-w-md shadow-brutal" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              {modalError && <p className="font-mono text-xs text-red-400 bg-red-950/30 border border-red-900 px-3 py-2">{modalError}</p>}
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={isCreate ? handleCreate : handleEdit} disabled={savingGallery}
                  ...
                  {savingGallery ? 'Salvando...' : isCreate ? 'Criar' : 'Salvar'}
                </button>
              </div>
            </div>
          </div>
