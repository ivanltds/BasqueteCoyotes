...
export default function BaskferiaTournamentsSection({ initialTournaments }: Props) {
  ...
  function renderCompetidor(comp: Competidor | null, score: number | null, isWinner: boolean) {
    if (!comp) {
      return (
        <div className="flex items-center justify-between bg-b-dark/40 border border-dashed border-b-stone/20 p-3 h-14">
    ...
    return (
      <div 
        className={`flex items-center justify-between p-3 h-14 border transition-colors ${
          hasLost 
        ...
        <div className="flex items-center gap-2 min-w-0">
          {comp.photo_url ? (
            <img 
              src={comp.photo_url} 
              alt={comp.name} 
              className="w-8 h-8 object-cover border border-b-stone/20 shrink-0" 
            />
          ) : comp.team_logo_url ? (
            <img 
                      ...
                      <div className="col-span-7 flex items-center gap-3">
                        {item.photo_url ? (
                          <img 
                            src={item.photo_url} 
                            alt={item.name} 
                            className="w-10 h-10 object-cover border border-b-stone/20 shrink-0" 
                          />
                        ) : item.team_logo_url ? (
                          <img 
